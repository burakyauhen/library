...
class ProfileModal extends Modal {
        ...
        <div class="modal-profile__right-column">
          ...
          <div class="statistic statistic_modified" data-state="registered">  
            ...
            <div class="statistic__books statistic__books_modified">
              ...
              <p class="statistic__count-books"></p>
            </div>
          </div>
  ...
  private fillModal(user: User) {
    ...
    const countBooks = document.querySelectorAll('.statistic__count-books')[1] as HTMLElement;
    ...
    const listOfRentedBooks = document.querySelector('.modal-profile__list') as HTMLUListElement;
    user.rentedBooks.forEach(book => {
      listOfRentedBooks.insertAdjacentHTML('beforeend', `<li>${book}</li>`)
    });
    countVisits.textContent = `${user.visits}`;
    countBooks.textContent = `${user.rentedBooks.length}`;
